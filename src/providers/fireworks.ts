import type { ChatCompletionRequest, ChatCompletionResponse, ChatMessage, RawChatCompletionResponse, RawChoice, RawUsage, Usage } from "../types";
...
export class FireworksProvider extends BaseLLMProvider {
  ...
  async complete(request: ChatCompletionRequest): Promise<ChatCompletionResponse> {
    ...
    return { id: data.id || this.generateId(), model: data.model || request.model, choices: data.choices?.map((c: RawChoice) => ({ index: c.index||0, message: { role: (c.message?.role as ChatMessage['role'])||'assistant', content: c.message?.content||'' }, finishReason: (c.finish_reason as ChatCompletionResponse['choices'][0]['finishReason'])||'stop' }))||[], usage: data.usage ? { promptTokens: data.usage.prompt_tokens||0, completionTokens: data.usage.completion_tokens||0, totalTokens: data.usage.total_tokens||0 } : undefined, created: Date.now() };
  }
}
