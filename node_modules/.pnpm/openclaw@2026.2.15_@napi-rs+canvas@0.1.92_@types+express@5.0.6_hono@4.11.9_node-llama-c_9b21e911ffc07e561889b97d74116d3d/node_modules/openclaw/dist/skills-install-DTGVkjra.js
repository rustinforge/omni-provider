...
import { b as resolveSkillKey, i as loadWorkspaceSkillEntries, t as resolveSkillsInstallPreferences } from "./skills-Bme2RWJt.js";
import { t as fetchWithSsrFGuard } from "./fetch-guard-BlxX1n2G.js";
import { t as resolveBrewExecutable } from "./brew-BaEuKjeL.js";
import { n as resolveSafeBaseDir, t as isWithinDir } from "./path-safety-CP6a8uVu.js";
import { a as extractArchive$1, r as safePathSegmentHashed } from "./install-safe-path-BA1GPy8X.js";
...
function summarizeInstallOutput$1(text) {
	const raw = text.trim();
	if (!raw) return;
...
function formatInstallFailureMessage$1(result) {
	const code = typeof result.code === "number" ? `exit ${result.code}` : "unknown exit";
	const summary = summarizeInstallOutput$1(result.stderr) ?? summarizeInstallOutput$1(result.stdout);
...
async function installDownloadSpec(params) {
	...
	return {
		ok: success,
		message: success ? `Downloaded and extracted to ${targetDir}` : formatInstallFailureMessage$1(extractResult),
